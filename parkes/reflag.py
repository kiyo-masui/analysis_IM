...
def flag(Data, NoiseData, thres=3.0, max_noise_factor=-1, modes_subtract=1,
    ...
    for jj in range(n_time):
        if sp.all(un_mask[jj,...] == un_mask[jj,0,0,0,0]):
            continue
        else:
            all_masks_same = False
            break
    if all_masks_same:
        polys = misc.ortho_poly(time, n_polys, un_mask[:,0,0,0,0], 0)
        polys.shape = (n_polys, len(time), 1, 1, 1, 1)
    else:
        polys = misc.ortho_poly(time[:,None,None,None,None], n_polys, un_mask, 0)
    # Subtract the slope mode (1th mode) out of the NoiseData.
    amps = sp.sum(data * un_mask * polys, 1)
    amps *= subtract_weights[:,None,None,None,None]
    ...
    for threshold in thresholds:
        ...
        # If any beam, polarization or cal state is masked, they all should be.
        bad_inds = sp.any(sp.any(sp.any(bad_inds, 1), 1), 1)
        mask[bad_inds[:,None,None,None,:]] = True
    ...
    # Flag any time that is an outlier (for any beam, polarization or cal state).
    bad_times = sp.any(sp.any(sp.any(abs(fmean) > thres * fmean_std, 1), 1), 1)
    mask[bad_times,:,:,:,:] = True
    ...
    if max_noise_factor > 0:
        ...
        for ii in range(3):
            ...
            bad_chans = sp.any(sp.any(sp.any(vars > max_noise_factor * 
                mean_vars[:,:,:,None], 0), 0), 0)
            mask[:,:,:,:,bad_chans] = True
