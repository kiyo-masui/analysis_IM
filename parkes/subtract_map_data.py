...
def sub_map(Data, Maps, correlate=False, pols=(), beams=(),  make_plots=False,
    ...
    if len(beams) == 0 or beams is None:
        beams = range(Data.dims[1])
    elif not hasattr(beams, '__iter__'):
        beams = (beams, )
    
    # Convert pols to an interable.
    # for Parkes data, the pol is the 3th axis
    if pols is None :
        #pols = range(Data.dims[1]) # for GBT
        pols = range(Data.dims[2])  # for Parkes
    elif not hasattr(pols, '__iter__') :
        pols = (pols, )
    elif len(pols) == 0 :
        #pols = range(Data.dims[1]) # for GBT
        pols = range(Data.dims[2])
    # If solving for gains, need a place to store them.
    if correlate :
        #out_gains = sp.empty((len(pols),) + Data.dims[2:4]) # GBT
        out_gains = sp.empty((13, len(pols),) + Data.dims[2:4]) # Parkes
    for beam_ind in beams:
        for pol_ind in pols :
            ...
            if interpolation == 'nearest' :
                ...
                ra_ind = map.tools.calc_inds(Data.ra[:,beam_ind], centre[1], shape[1],
                                             spacing[1])
                dec_ind = map.tools.calc_inds(Data.dec[:,beam_ind], centre[2], shape[2],
                                              spacing[2])
                ...
                on_map_inds = sp.logical_and(
                    sp.logical_and(Data.ra[:,beam_ind] > min(map_ra), 
                                   Data.ra[:,beam_ind] < max(map_ra)),
                    sp.logical_and(Data.dec[:,beam_ind] > min(map_dec), 
                                   Data.dec[:,beam_ind] < max(map_dec)))
                ...
                for ii in range(len(on_map_inds)) :
                    if on_map_inds[ii] :
                        submap[:, jj] = Map.slice_interpolate([1, 2], 
                                [Data.ra[ii,beam_ind], Data.dec[ii,beam_ind]], 
                                kind=interpolation)
                        jj += 1
            ...
            for cal_ind in range(Data.dims[3]) :
                data = Data.data[:, beam_ind, pol_ind, cal_ind, :]
                ...
                if correlate :
                    ...
                    gain[counts == -1] = 0.0
                    out_gains[beam_ind, pol_ind,cal_ind,:] = gain
                else :
                    gain = 1.0
