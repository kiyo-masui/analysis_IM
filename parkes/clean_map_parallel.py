...
class CleanMapMaker(object) :
    ...
    def mpiexecute(self, nprocesses=1):
        ...
        for pol_str in self.params['polarizations']:
            for band in bands:
                if self.params['save_noise_diag']:
                    clean_map, noise_diag, noise_idiag =\
                            self.execute(pol_str, band, comm, rank, size)
                else:
                    clean_map = self.execute(pol_str, band, comm, rank, size)

                if rank == 0:
                    for i in range(1, size):
                        clean_map += comm.recv(source=i, tag=11)
                        if self.params['save_noise_diag']:
                            noise_diag += comm.recv(source=i, tag=12)
                            noise_idiag += comm.recv(source=i, tag=13)
                    ...
                    out_fname = (self.params['output_root'] + 'clean_map_'
                    ...
                    if self.params['save_noise_diag'] :
                        noise_diag_fname = (self.params['output_root'] + 'noise_diag_'
                                            + pol_str + band_str + '.npy')
                        algebra.save(noise_diag_fname, noise_diag)
                        noise_diag_fname = (self.params['output_root'] + 'noise_inv_diag_'
                                            + pol_str + band_str + '.npy')
                        algebra.save(noise_diag_fname, noise_idiag)
                    ...
                    comm.ssend(clean_map, dest=0, tag=11)
                    if self.params['save_noise_diag']:
                        comm.ssend(noise_diag, dest=0, tag=12)
                        comm.ssend(noise_idiag, dest=0, tag=13)
                    comm.barrier()
    ...
    def execute(self, pol_str, band, comm=None, rank=0, size=1, nprocesses=1):
        """Worker funciton."""
        params = self.params
        ...
        dmap_fname = (in_root + 'dirty_map_' + pol_str + band_str + '.npy')
        #all_in_fname_list.append( kiyopy.utils.abbreviate_file_path(dmap_fname))
        # Load the dirty map 
        dirty_map = algebra.load(dmap_fname)
        dirty_map = algebra.make_vect(dirty_map)
        if dirty_map.axes != ('freq', 'ra', 'dec') :
            msg = ("Expeced dirty map to have axes ('freq','ra', 'dec'),"
                    " but it has axes: " + str(dirty_map.axes))
        ...
        if save_noise_diag :
            ...
            noise_idiag = algebra.zeros_like(clean_map)
        ...
        if comm != None:
            ...
            for ii in range(dirty_map.shape[0])[rank::size] :
                noise_fname = (in_root + 'noise_inv_' + pol_str +
                               band_str + '/%03d.npy'%ii)
                #all_in_fname_list.append(
                #    kiyopy.utils.abbreviate_file_path(noise_fname))
                noise_inv_freq = algebra.load(noise_fname)
                noise_inv_freq = algebra.make_mat(noise_inv_freq)
                #if self.feedback > 1:
                if noise_inv_freq.axes != ('freq', 'ra', 'dec', 'ra', 'dec'):
                    msg = ("Expeced noise matrix to have axes "
                           "('freq', 'ra', 'dec', 'ra', 'dec'), "
                           "but it has: " + str(noise_inv_freq.axes))
                    raise ce.DataError(msg)
                ...
                ##    sys.stdout.flush()
                noise_inv_freq.shape = (shape[1]*shape[2], shape[1]*shape[2])
                ...
                if self.feedback > 1:
                    print "RANK: %03d"%rank,\
                        ...
                        "\n      Using noise inverse: " + noise_fname,\
                        "\n      Frequency: %03d[%5.1f]"%(ii, frequencies[ii]),\
                        ", discarded: ",\
                        "%4.1f" % (100.0 * sp.sum(bad_modes)/bad_modes.size),\
                        "% of modes\n\n"
                    sys.stdout.flush()
            if save_noise_diag:
                return clean_map, noise_diag, noise_idiag
            else:
                return clean_map
            # parallel map making stop and return here. 

       #         elif params["from_eig"]:
