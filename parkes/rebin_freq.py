...
def rebin(Data, width, mean=False, by_nbins=False) :
    ...
    if by_nbins :
        ...
        if Data.dims[-1] % width == 0:
            ...
            new_data = ma.empty(Data.dims[:4] + (nbins,))
            # Loop over new bins and rebin.
            for ii in xrange(nbins) :
                new_data[:,:,:,:,ii] = method(
                        Data.data[:,:,:,:,ii*width:(ii+1)*width],4)
