...
from core import base_fits as bf
...
fields_and_axes = { 
                   ...
                   'CRVAL2' : ('time', 'beam' ), # Azimuth
                   'CRVAL3' : ('time', 'beam' ), # Elevation
                   # Polarization indicies
                   'CRVAL4' : ('pol', ),
                   'CAL' : ('cal', ),
                   'BEAM' : ('beam', ),
                   'TSYS' : ('time', 'beam', 'pol', 'cal'),
                   'RA' : ('time', 'beam'), # the ra and dec read from fits
                   'DEC' : ('time', 'beam'),
...
class Reader(object) :
    ...
    def get_scan_IF_inds(self, scan_ind, IF_ind) :
        ...
        # Find all the records that correspond to this IF, this scan and this beam.
        ...
        nbeam= len(sp.unique(self.fitsdata.field('BEAM')[inds_sif]))
        ...
        ntimes = len(inds_sif)//npol//ncal//nbeam
        inds_sif = sp.reshape(inds_sif, (ntimes, nbeam, npol, ncal))
        ...
        if self.verify_ordering > 0:
            ...
            for ii in range(ntimes) :
                ...
                try :
                    thisLST = self.fitsdata.field('LST')[inds_sif[ii,0,0,0]]
    ...
    def read(self, scans=None, bands=None, force_tuple=False, IFs=None) :
        ...
        beams : tuple of intergres
            Which beam of feed to process. A list of integers with 0
            coorsponding to feed 1(the center one). Default is all of them
                ...
                for field, axis in fields_and_axes.iteritems() :
                    ...
                    if axis :
                        ...
                        Data_sif.set_field(field, self.fitsdata.field(field)
                            [inds_sif[0,0,0,0]], axis, field_format)
...
class Writer() :
    ...
    def _add_single_block(self, Block) :
        ...
        n_records = dims[0]*dims[1]*dims[2]*dims[3]
        ...
        if self.first_block_added :
            self.data = data.reshape((n_records, dims[-1]))
        else :
            self.data = sp.concatenate((self.data, data.reshape((
                                        n_records, dims[-1]))), axis=0)
        ...
        for field, axes in Block.field_axes.iteritems() :
            ...
            #print field, broadcast_shape, axes
