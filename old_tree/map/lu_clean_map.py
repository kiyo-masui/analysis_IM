    ...
    def execute(self, nprocesses=1):
        ...
        for pol_str in params['polarizations']:
            for band in bands:
                ...
                print "LU decomposition and solving."
                lu_decomp,clean_map = psroutine.pdgetrf_pdgetrs(Mat,
                          Rhs, destroy_mat=True, destroy_rhs=True)
                print "Gotten clean map."

                lu_fname = (out_root + 'noise_inv_lu_' + pol_str + "_" +
                ...
                if rank == 0 and self.feedback > 1:
                    print "Writing LU decomposition to " + lu_fname
                lu_decomp.to_npy(lu_fname, fortran_order=False)#,
