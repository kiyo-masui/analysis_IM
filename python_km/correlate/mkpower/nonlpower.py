...
def windowfunction(k, A):
	return 1.e6*A[0]/(1. + (k/(1.e-3*A[1]))**2 + (k/(1.e-3*A[2]))**4)
...
class TheoryPowerSpectrumMaker(object):
	...
	def execute(self, nprocesses=1):
		...
		PKcamb_fname = in_root + 'PKcamb.npy'
		...
		non0 = yy[0].nonzero()
		y = yy[0].take(non0)[0][:-5]
		x = xx[0].take(non0)[0][:-5]
		non0 = yy[-1].nonzero()
		y = np.append(y, yy[-1].take(non0)[0][5:-4])
		x = np.append(x, xx[-1].take(non0)[0][5:-4])
		err = y.copy()*10.
		err[5:] = err[5:]*1.e-8
