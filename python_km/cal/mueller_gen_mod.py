...
#       ch = 0
        ...
        for i in range(0,len(t),4):
            t[i] = 1*Isrc+Qsrc*((0.5*dG*sp.cos(2.*al)-2.*ep*sp.cos(ph-ch)*sp.sin(2.*al))*cos(2.*theta[i])-(0.5*dG*sp.sin(2.*al)*sp.cos(ch)+2*ep*(sp.cos(al)*sp.cos(al)*sp.cos(ph)-sp.sin(al)*sp.sin(al)*sp.cos(ph-2.*ch)))*sp.sin(2.*theta[i]))+Usrc*((0.5*dG*sp.cos(2.*al)-2.*ep*sp.sin(2.*al)*sp.cos(ph-ch))*sp.sin(2.*theta[i])+((0.5*dG*sp.sin(2.*al)*sp.cos(ch)+2*ep*(sp.cos(al)*sp.cos(al)*sp.cos(ph)-sp.sin(al)*sp.sin(al)*sp.cos(ph-2.*ch)))*sp.sin(2.*theta[i])))   
            t[i+1] = 0.5*dG*Isrc + Qsrc*(sp.cos(2.*al)*sp.cos(2.*theta[i])-sp.sin(2*al)*sp.cos(ch)*sp.sin(2*theta[i]))+Usrc*(sp.cos(2*al)*sp.cos(2*theta[i])+sp.sin(2*al)*sp.cos(ch)*sp.sin(2*theta[i]))
            t[i+2] = 2*ep*sp.cos(ps+ph)*Isrc+Qsrc*(-sp.sin(2*al)*sp.cos(ps+ch)*sp.cos(2.*theta[i])-(sp.cos(al)*sp.cos(al)*sp.cos(ps)-sp.sin(al)*sp.sin(al)*sp.cos(ps+2*ch))*sp.sin(2*theta[i]))+Usrc*(-sp.sin(2*al)*sp.cos(ps+ch)*sp.cos(2*theta[i])+(sp.cos(al)*sp.cos(al)*sp.cos(ps)-sp.sin(al)*sp.sin(al)*sp.cos(ps+2*ch))*sp.sin(2*theta[i]))
            t[i+3] = 2*ep*sp.sin(ps+ph)*Isrc+Qsrc*(-sp.sin(2*al)*sp.sin(ps+ch)*sp.cos(2*theta[i])-(sp.sin(ps)*sp.cos(al)*sp.cos(al)-sp.sin(al)*sp.sin(al)*sp.sin(ps+2*ch))*sp.sin(2*theta[i]))+Usrc*(-sp.sin(2*al)*sp.sin(ps+ch)*sp.cos(2*theta[i])+(sp.sin(ps)*sp.cos(al)*sp.cos(al)-sp.sin(al)*sp.sin(al)*sp.sin(ps+2*ch))*sp.sin(2*theta[i]))
        return t 
...
#        d[k,:] = sp.tan(2*33*sp.pi/180) # the 33 degrees here  is specific to 3C286, takes a different value for different calibrators
        ...
        p0 = [0.3, 90.0, 170.0, 10.0, 0.016, 0] # preliminary values based on guesses and heiles generation.
        error = sp.ones(4*self.file_num)
        #Note that error can be used to weight the equations if not all set to one.

        p_val_out = sp.zeros((freq_len, 7))
