...
im2 = ax2.imshow(s_af.T, extent = (500.0, 900.0, 0, 5.0), aspect= 'auto', vmin = mint, vmax=maxt, cmap = cm)
...
ps.nu_lower, ps.nu_upper, ps.nu_num = (500, 900.0, 128)
#ps.flux_max = 50.0
psm = ps.getfield()


s_aa = psm[0,:,:]
s_af = psm[:,:10,:].sum(axis = 1) ** fs[:,np.newaxis]**0.7

print s_af.min(), s_af.max()
...
ax1.imshow(s_aa.T, extent = (0, 5.0, 0, 5.0), aspect= 'equal', cmap = cm, vmin = 0, vmax = 1.5)
ax1.set_ylabel("y / degrees")
ax1.set_xlabel("x / degrees")
ax2 = f.add_subplot(121)
...
im2 = ax2.imshow(s_af.T, extent = (500.0, 900.0, 0, 5.0), aspect= 'auto', cmap = cm, vmin = 0, vmax = 1.5)
ax2.set_ylabel("y / degrees")
ax2.set_xlabel("Frequency / MHz")
