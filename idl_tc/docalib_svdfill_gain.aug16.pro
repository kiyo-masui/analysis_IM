pro docalib_svdfill_gain,radio,weight,t_off,radio_on,weight_on,field=field,fn=fn,sigmacut=sigmacut,dname=dname,svdfg=svdfg,fitsfile=fitsfile,sim=sim,signal=signal
...
;      endfor
      ;
      fdir='/cita/d/raid-cita/tchang/GBT/Processed_sim/'+dname+'/'+field+'_'+ffdir(ifile)+'/'
      spawn,'mkdir '+fdir
      ;
   endif
   ;
 ...
 ;     freqnum=freqnum>1
      ...
      ;
      tfradio=dblarr(ntime*nscan,nz)
      thermal=(dnu*dt)*(flagtot20mode>1)
      for ifreq=0,nz-1 do begin
         tfweight=(1./variance(radiores20mode(*,0,ifreq)))<(thermal(*,0,ifreq))
      ...
      ;
      tfradio=dblarr(ntime*nscan,nz)
      thermal=(dnu*dt)*(flagtot25mode>1)
      for ifreq=0,nz-1 do begin
         tfweight=(1./variance(radiores25mode(*,0,ifreq)))<(thermal(*,0,ifreq))
      ...
      ;
      tfradio=dblarr(ntime*nscan,nz)
      thermal=(dnu*dt)*(flagtot10mode>1)
      for ifreq=0,nz-1 do begin
         tfweight=(1./variance(radiores10mode(*,0,ifreq)))<thermal(*,0,ifreq)
