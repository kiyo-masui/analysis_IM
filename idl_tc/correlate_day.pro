PRO correlate_day,radio,weight,optical,field=field,fin=fin,radiocut=radiocut,fdir=fdir,noise=noise,sim=sim,daisy=daisy,outfile=outfile,dname=dname,svd=svd
...
;   tmp=dblarr(npol,nx,ny,nz)
   openr,1,ffdir+field+'.radiotot_weighted_aftsvd'
   readf,1,radio
   close,1
;   radioall(i,*,*,*)=tmp
   ;
   openr,1,ffdir+field+'.weighttot_aftsvd'
   readf,1,weight
   close,1
;   weightall(i,*,*,*)=tmp
   ;
