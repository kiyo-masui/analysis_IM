pro docalib_svdfill_gain_new,radio,weight,t_off,radio_on,weight_on,field=field,fn=fn,sigmacut=sigmacut,dname=dname,svdfg=svdfg,fitsfile=fitsfile
...
noisetot=noiseradio
radiotot2mode=radio
weighttot2mode=weight
radiotot3mode=radio
weighttot3mode=weight
...
;   fsdir='/cita/scratch/cottontail/tchang/GBT/Processed/'+dname+'/svd_nominus1/'+field+'_'+ffdir(ifile)+'/'
   spawn,'mkdir '+fdir
   ...
   radio=dblarr(Npol,nx,ny,nz)
   weight=radio
   radio3mode=dblarr(Npol,nx,ny,nz)
   weight3mode=radio
   radio2mode=dblarr(Npol,nx,ny,nz)
   weight2mode=radio
   noiseradio=dblarr(Npol,nx,ny,nz)
   noiseradio2mode=dblarr(Npol,nx,ny,nz)
   noiseradio3mode=dblarr(Npol,nx,ny,nz)
...
;      device,/close
      ;
      device,filename=fdir+'Psky_freq_matched'+strtrim(string(scan),2)+'.ps'
      ...
      close,1
      ;
      openw,1,fdir+'crosspol_tf_b4svd'+scann
      printf,1,time_freq_crosspol
      close,1
      ;
      openw,1,fdir+'freqtot'+scann
      printf,1,freqtot
      close,1
      ;
      openw,1,fdir+'ra'+scann
      printf,1,ra
      close,1
      ;
      openw,1,fdir+'dec'+scann
      printf,1,dec
      close,1
      ;
      ;
      ;
      device,filename=fdir+'Pnoise_freq_matched'+strtrim(string(scan),2)+'.ps'
      ...
      close,1
      ;
      openr,1,fdir+'crosspol_tf_b4svd'+scann
      readf,1,time_freq_crosspol
      close,1
      ;
   ...
   endelse
      ...
      if keyword_set(svdfg) then begin
         ...
         flagtot2mode=flagtot
         flagtot3mode=flagtot
         ;
         time_freq_flag2mode=time_freq_flag
         time_freq_flag3mode=time_freq_flag
         ;
         ;
         ; svd
         ...
         radiores3mode = time_freq_flag-radiosvd3mode
         ;
         device,filename=fdir+'Psky_svd1'+strtrim(string(scan),2)+'.ps'
;         vfil=v
;         vfil(15:*,*)=0
;         plt_image, transpose(reform(time_freq_flag(*,2,*))),/scalable,/colbar
...
;         plt_image,
;         transpose(reform(radiosvd5mode(*,2,*))),/scalable,/colbar
...
;            time_freq_noise(ind)=0.
         ...
         ;
         ind=where(abs(radiores3mode) gt (sigma_th*sigma),cind)
         if (cind gt 0) then begin
            radiores3mode(ind)=0
            flagtot3mode(ind)=0
;            time_freq_noise(ind)=0.
...
;         if (cind gt 0) then radiores2mode(ind)=0
         ...
         plt_image, transpose(reform(radiores3mode(*,0,*))),/scalable,/colbar
...
;         plt_image, transpose(reform(radiores10mode(*,0,*))),/scalable,/colbar
         ...
         ;
         ind=where(flagtot2mode eq 0, cind)
         time_freq_flag2mode(ind)=radiosvd2mode(ind)
         ;
         ind=where(flagtot3mode eq 0, cind)
         time_freq_flag3mode(ind)=radiosvd3mode(ind)
         ;
         ; do frequency weighting
         freqweight=dblarr(npol,nfreq*3/5*nwin)
         freqweight2mode=dblarr(npol,nfreq*3/5*nwin)
         freqweight3mode=dblarr(npol,nfreq*3/5*nwin)
         for ii=0,npol-1 do begin
            for kk=0,nfreq*3/5*nwin-1 do begin
               ...
               ;
               ind=where(flagtot2mode(*,ii,kk) eq 1,cind)
               if cind gt 1 then begin
                  freqweight2mode(ii,kk)=variance(radiores2mode(ind,ii,kk))
                  time_freq_flag2mode(ind,ii,kk)=time_freq_flag2mode(ind,ii,kk)/freqweight2mode(ii,kk)
               endif
               ;
               ind=where(flagtot3mode(*,ii,kk) eq 1,cind)
               if cind gt 1 then begin
                  freqweight3mode(ii,kk)=variance(radiores3mode(ind,ii,kk))
                  time_freq_flag3mode(ind,ii,kk)=time_freq_flag3mode(ind,ii,kk)/freqweight3mode(ii,kk)
               endif
               ;
            endfor
         endfor
...
;            radiosvd2mode(*,ii,*) = u ## diag_matrix(w2) ## transpose(v)
            radiosvd5mode(*,ii,*) =u ## diag_matrix(w5) ## transpose(v)
;            radiosvd10mode(*,ii,*) = u ## diag_matrix(w10) ##
;            transpose(v)
            ;
            radio2d=reform(time_freq_flag2mode(*,ii,*))
            la_svd,radio2d, s,u,v,/double,status=status
            w2=dblarr(n_elements(s))
            w2(0:1)=s(0:1)
            radiosvd2mode(*,ii,*) = u ## diag_matrix(w2) ## transpose(v)
            ;
            radio2d=reform(time_freq_flag3mode(*,ii,*))
            la_svd,radio2d, s,u,v,/double,status=status
            w3=dblarr(n_elements(s))
            w3(0:2)=s(0:2)
            radiosvd3mode(*,ii,*) = u ## diag_matrix(w3) ## transpose(v)
            ;        
            ;
         ...
         radiores = time_freq_flag-radiosvd
         radiores1mode = time_freq_flag-radiosvd1mode
         radiores2mode = time_freq_flag2mode-radiosvd2mode
         radiores5mode = time_freq_flag-radiosvd5mode
         radiores3mode = time_freq_flag3mode-radiosvd3mode
         ; de-freq weight
         for ii=0,npol-1 do begin
            for kk=0,nfreq*3/5*nwin-1 do begin
               ind=where(flagtot(*,ii,kk) eq 1,cind)
               ...
               ind=where(flagtot2mode(*,ii,kk) eq 1,cind)
               if cind gt 1 then begin
                  radiores2mode(ind,ii,kk)=radiores2mode(ind,ii,kk)*freqweight2mode(ii,kk)
               endif
               ind=where(flagtot3mode(*,ii,kk) eq 1,cind)
               if cind gt 1 then begin
                  radiores3mode(ind,ii,kk)=radiores3mode(ind,ii,kk)*freqweight3mode(ii,kk)
               endif
            endfor
         endfor
         ; do sigma flagging
...
;         plt_image, transpose(reform(radiores10mode(*,0,*))),/scalable,/colbar
         ...
         device,/close
         ;
         device,filename=fdir+'Pnoise_flag'+strtrim(string(scan),2)+'.ps'
         plt_image,transpose(reform(time_freq_noise(*,0,*))),/scalable,/colbar
         plt_image,transpose(reform(time_freq_noise(*,1,*))),/scalable,/colbar
         plt_image,transpose(reform(time_freq_noise(*,2,*))),/scalable,/colbar
         plt_image,transpose(reform(time_freq_noise(*,3,*))),/scalable,/colbar
      ...
      help,flagtot
      ;
      mk_hicube_noise,radiores5mode,flagtot,time_freq_noise,ra,dec,freqtot,pol,$
                         parallatic,radio=radio,weight=weight,$
                         noiseradio=noiseradio,field=field,flagnum=flagnumoff
      mk_hicube_noise,radiores2mode,flagtot2mode,time_freq_noise,ra,dec,freqtot,pol,$
                         parallatic,radio=radio2mode,weight=weight2mode,$
                         field=field,noiseradio=noiseradio2mode,flagnum=flagnumoff2mode
      mk_hicube_noise,radiores3mode,flagtot3mode,time_freq_noise,ra,dec,freqtot,pol,$
                      parallatic,radio=radio3mode,weight=weight3mode,$
                      field=field,noiseradio=noiseradio3mode,flagnum=flagnumoff3mode
...
;             field=field,kstart=kstart,kend=kend,flagnum=flagnumon
      ...
      ;
      device,filename=fdir+'mk_hicub_radio_aftsvd'+strtrim(string(scan),2)+'.ps'
   ...
   radiotot2mode=radiotot2mode+radio2mode
   weighttot2mode=weighttot2mode+weight2mode
   radiotot3mode=radiotot3mode+radio3mode
   weighttot3mode=weighttot3mode+weight3mode
   ;
   ...
   radio2(ind)=radio2(ind)/weight2mode(ind)
   ;
   ...
   radio2(ind)=radio2(ind)/weight3mode(ind)
   ;
   openw,1,fdir+field+'.radio3mode_weighted_aftsvd'
   printf,1,radio2
   close,1
   ;
   device,filename=fdir+'Psky_aftsvd.ps'
   radim1=reform(radio2(0,*,*,*))
   radim2=reform(radio2(1,*,*,*))
   radim3=reform(radio2(2,*,*,*))
   radim4=reform(radio2(3,*,*,*))
...
printf,1,radiotot2mode
close,1
;
openw,1,fdir+field+'.weighttot2mode_aftsvd'
printf,1,weighttot2mode
close,1
;
ind=where(weighttot2mode gt 0,cind)
radiotot2mode(ind)=radiotot2mode(ind)/weighttot2mode(ind)
;
...
printf,1,radiotot3mode
close,1
;
openw,1,fdir+field+'.weighttot3mode_aftsvd'
printf,1,weighttot3mode
close,1
;
ind=where(weighttot3mode gt 0,cind)
radiotot3mode(ind)=radiotot3mode(ind)/weighttot3mode(ind)
;
...
radim1=reform(radiotot(0,*,*,*))
radim2=reform(radiotot(1,*,*,*))
radim3=reform(radiotot(2,*,*,*))
radim4=reform(radiotot(3,*,*,*))
radim1=reform(radim1,nx*ny,nz,/overwrite)
radim2=reform(radim2,nx*ny,nz,/overwrite)
radim3=reform(radim3,nx*ny,nz,/overwrite)
radim4=reform(radim4,nx*ny,nz,/overwrite)
