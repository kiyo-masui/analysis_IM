PRO correlate_gain_input,corrv,corre,radio,weight,optical,field=field,fin=fin,fdir=fdir,noise=noise,sim=sim,daisy=daisy,outfile=outfile,dname=dname,svdfg=svdfg,mode=mode,fixweight=fixweight,wnum=wnum
seed=1
...
;   radioall(i,*,*,*)=tmp
   ;
;weight=make_array(1,nx,ny,nz,value=1.)
;openw,1,ffdir+field+'.simweighttot'
...
;
if not keyword_set(wnum) then wnum=''
;
device,filename='weight'+strtrim(string(wnum),2)+'.ps'
ind=where(1./correrrslice ne 0 and finite(correrrslice))
ind2=where(1./correrrslice(ind) lt 1e9)
corrplot=dblarr(n_elements(correrrslice))
corrplot(ind(ind2))=(1./correrrslice(ind(ind2)))
corrplot=corrplot+(10.^(-4.))
plot,corrplot,/ylog
device,/close
;
if not keyword_set(fixweight) then begin
   openw,1,'correrrslice.dat'
   printf,1,correrrslice
   close,1
endif else begin
   openr,1,'correrrslice.dat'
   readf,1,correrrslice
   close,1
endelse
;
