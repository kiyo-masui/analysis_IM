...
def bin_map(catalog, num_bins, group, subgroup):
    ...
    # change zedges to xedges since issue with JD box slicing
    xedges = np.linspace(0, posx.max(), num_bins)
    yedges = np.linspace(0, posy.max(), num_bins)
    zedges = np.linspace(0, posx.max(), num_bins)
    sample = np.zeros((len(posx), 4))
    sample[:, 0] = posx
    sample[:, 1] = posy
    sample[:, 2] = posz
    ...
    y_centre = posy.max()/2.
    z_centre = posz.max()/2.
    # change in freq from z to x since issue with JD box slicing
    info = {'ra_delta': x_delta,
            'dec_delta': y_delta,
            'dec_centre': y_centre,
            'axes': ('freq', 'ra', 'dec'),
            'ra_centre': x_centre,
            'freq_centre': x_centre,
            'freq_delta': x_delta,
            'type': 'vect'}
    return binning.histogram3d_weights(sample, xedges, yedges, zedges), info


def bin_val_map(catalog, num_bins, group, subgroup):
    """
    Read h5py catalog, return binned algebra map without making deltas.
    Output: map,info.
    """
    ...
    file.close()
    boolx = (posx <= 1000.)
    print "boolx",len(boolx)
    booly = (posy <= 1000.)
    print "booly",len(booly)
    boolz = (posz <= 1000.)
    print "boolz",len(boolz)
    ...
    # create info for binning function
    # change zedges to xedges since issue with JD box slicing
    xedges = np.linspace(0, 1000., num_bins)
    yedges = np.linspace(0, 1000., num_bins)
    zedges = np.linspace(0, 1000., num_bins)
    sample = np.zeros((len(posx), 4))
    sample[:, 0] = posx
    sample[:, 1] = posy
    sample[:, 2] = posz
    sample[:, 3] = weights
    # create info for pipeline
    x_delta = 1000./num_bins
    y_delta = 1000./num_bins
    z_delta = 1000./num_bins
    x_centre = 1000./2.
    y_centre = 1000./2.
    z_centre = 1000./2.
    # change in freq from z to x since issue with JD box slicing
...
def join_index_delta_algebra(catalog_list, num_bins, group, subgroup,
                       directory, savefile):
    binned_map, info = bin_map(catalog_list[0], num_bins, group, subgroup)
    for num in range(1,len(catalog_list)):
        binned_map = binned_map + bin_map(catalog_list[num], num_bins, group,
