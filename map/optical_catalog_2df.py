...
def bin_catalog_data(catalog, freq_axis, ra_axis,
    ...
    #select only a sub-set of the full catalog
    ind = np.arange(num_catalog)
    random.shuffle(ind)
    sample = sample[ind[:(num_catalog/2)]]
...
def convert_to_physical_coordinate(freq, ra, dec, ra_fact):
    ...
    x = c * np.cos(dec) * np.cos(ra)
    y = c * np.cos(dec) * np.sin(ra)
    z = c * np.sin(dec)
...
def bin_catalog_file(filename, freq_axis, ra_axis, dec_axis, 
    ...
    if physical_cube:
        print "convert to physical coordinate"
        if ra_centre != None:
...
class Bin2dF(object):
    def __init__(self, parameter_file=None, params_dict=None, use_mocks=True, feedback=0):
        ...
        self.ra_centre = copy.deepcopy(self.template_map.info['ra_centre'])
        self.dec_centre = copy.deepcopy(self.template_map.info['dec_centre'])

        if self.params['physical_cube']:
            self.template_map = physical_gridding.physical_grid_largeangle(
                self.template_map, refinement=1, pad=5, order=1, info_only=True)
        ...
        if not use_mocks:
           self.selection_function = algebra.load(self.outfile_selection)
           self.separable_selection = algebra.load(self.outfile_separable)
    ...
    def realmap(self):
        ...
        self.realmap_binning, self.biasmap_binning\
            = bin_catalog_file(self.infile_data, self.freq_axis, 
                               self.ra_axis, self.dec_axis, 
                               physical_cube=self.params['physical_cube'], 
                               ra_centre=self.ra_centre, 
                               dec_centre=self.dec_centre,
                               skip_header=1, debug=False)
...
if __name__=="__main__":
    ...
    algebra.save('/mnt/raid-project/gmrt/goblot/2df_binning/tempfile', tempfile)

    map_dir = '/mnt/raid-project/gmrt/goblot/2df_binning/'
    ...
    #map_dir = '/mnt/scratch-gl/ycli/2df_catalog/map/map_full_selection_1000mock_physical/'
    #map_dir = '/mnt/scratch-gl/ycli/2df_catalog/map/map_2929.5_parkes_selection_1000mock/'
    if not os.path.exists(map_dir):
        os.makedirs(map_dir)

    for i in range(10):
        print "Tirage numero", i
        suffixe = '_ycli_50_%s'%i
        ...
        "outfile_data": map_dir + "real_map_2df"+suffixe+".npy",
        "outfile_bias": map_dir + "bias_map_2df.npy",
        "outfile_mock": map_dir + "mock_map_2df_%04d.npy",
        "outfile_deltadata": map_dir + "real_map_2df_delta"+suffixe+".npy",
        ...
        "template_file": "/mnt/raid-project/gmrt/goblot/2df_binning/tempfile",
        ...
        Bin2dF(params_dict=bin2dfparams_init,use_mocks=False).execute()
