...
select_type = 'transfer'

modes = [5, 10, 15, 20]
runStart = 502

runs = [x for x in np.arange(runStart, runStart+len(modes))]
#runs = [478, 479, 343, 368]
field = 'ra33'

inputroot = '/scratch2/p/pen/nluciw/parkes/analysis_IM/'
outputroot = '/scratch2/p/pen/nluciw/parkes/analysis_IM/field_%s_%s_select/'\
             %(field,select_type)
if not os.path.exists(outputroot):
    os.makedirs(outputroot)

ps_file = h5py.File(outputroot + 'ps_result.hd5', 'a')

rf_root = 'auto_rf_%dmode_2dpow'
tr_root = 'auto_tr_%dmode_2dpow'
si_root = 'auto_si_%dmode_2draw'
...
for i in range(len(modes)):
    print 'Loading', modes[i], 'mode(s).'
    ps_root = inputroot + 'full%s_auto_ps_%02dmode/'%(runs[i],modes[i]) + 'ps_result.hd5'
    ps_result = h5py.File(ps_root, 'r')

    power_spectrum_sim, k_mode_number_sim, sim_p_edges, sim_v_edges =\
        pss.load_power_spectrum('auto_si_%dmode_2dpow'%modes[i], ps_result)
    ...
    power_2d_raw_kmn_sim = ps_result['auto_si_%dmode_2draw_kmn'%modes[i]].value
    ...
    #auto_short_noise, short_noise_kmn, k_p_edges, k_v_edges =\
    #        pss.load_power_spectrum('auto_sn_%dmode_2dpow'%modes[i], ps_result)
    power_spectrum, k_mode_number, k_p_edges, k_v_edges =\
            pss.load_power_spectrum('auto_ps_%dmode_2dpow'%modes[i], ps_result)
    #power_spectrum -= auto_short_noise
    power_spectrum *= transfer_function**2
    pow.append(power_spectrum)

    #if modes[i] == 10:
    #    psa.plot_2d_power_spectrum(power_spectrum, None, k_p_edges, k_v_edges,
    #                           filename='comp_auto_ps_%dmode'%(modes[i]),
...
if select_type == 'transfer':
    for i in range(transfer_select.shape[0]):
        for j in range(transfer_select.shape[1]):
            ...
            print 'Options:', transfer[:,i,j]
            print 'Selected index', np.argmin\
                (np.abs(0.8 - transfer[:,i,j])), 'which is', transfer_select[i,j], \
                'at k_perp', k_v_edges[i], 'and k_parallel', k_p_edges[j]
            ...
            pow_select[i,j] = pow[np.argmin\
                (np.abs(0.8 - transfer[:,i,j])),i,j]
            mode_map[i][j] = modes[np.argmin(np.abs(0.8 - transfer[:,i,j]))]
...
psa.plot_2d_power_spectrum(mode_map, None, k_p_edges, k_v_edges, 
                       ...
                       logscale=False, cmax=20., cmin=0.)

psa.plot_2d_power_spectrum(np.mean(sim_select, axis=0), None, sim_p_edges, sim_v_edges,
                       filename='comp_auto_si_%d-%dmode'%(modes[0],modes[-1]),
                       label_list='', output=outputroot)
sim_2d = al.make_vect(np.mean(sim_select, axis =0), 
                      axis_names=power_spectrum_sim.info['axes'])
sim_2d.info = power_spectrum_sim.info
al.save_h5(ps_file, 'sim_2d', sim_2d)

psa.plot_2d_power_spectrum(std_select, None, k_p_edges, k_v_edges,
                           filename='comp_auto_std_%d-%dmode'%(modes[0],modes[-1]),
                           label_list='', output=outputroot)
std_2d = al.make_vect(std_select, axis_names=power_spectrum_sim.info['axes'])
std_2d.info = power_spectrum_sim.info
al.save_h5(ps_file, 'std_2d', std_2d)

psa.plot_2d_power_spectrum(pow_select, None, k_p_edges, k_v_edges,
                           filename='comp_auto_ps_%d-%dmode'%(modes[0],modes[-1]),
                           label_list='', output=outputroot)
ps_2d = al.make_vect(pow_select, axis_names=power_spectrum.info['axes'])
ps_2d.info = power_spectrum.info
al.save_h5(ps_file, 'ps_2d', ps_2d) 
...
filename = '%sauto_si_%d-%dmode_1dpow_positive.txt'%('', modes[0], modes[-1])
np.savetxt('%s/%s'%(outputroot, filename), si_positive.T, fmt='%15.12e')
filename = '%sauto_si_%d-%dmode_1dpow_negative.txt'%('', modes[0], modes[-1])
...
positive, negative = psa.seperate_positive_and_negative_power(ps_1d_mean,
                                                              ps_1d_std,
                                                              k_centre)
filename = '%sauto_ps_%d-%dmode_1dpow.txt'%('comp_varweight_', modes[0], modes[-1])
np.savetxt('%s/%s'%(outputroot, filename), np.concatenate((k_centre[None,:],\
        ps_1d_mean[None,:], si_1d_std[None,:]),axis=0).T, fmt='%15.12e')
filename = '%sauto_ps_%d-%dmode_1dpow_positive.txt'%('comp_varweight_', modes[0], modes[-1])
np.savetxt('%s/%s'%(outputroot, filename), positive.T, fmt='%15.12e')
filename = '%sauto_ps_%d-%dmode_1dpow_negative.txt'%('comp_varweight_', modes[0], modes[-1])
