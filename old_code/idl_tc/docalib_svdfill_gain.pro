pro docalib_svdfill_gain,radio,weight,t_off,radio_on,weight_on,field=field,fn=fn,sigmacut=sigmacut,dname=dname,svdfg=svdfg,fitsfile=fitsfile,sim=sim,signal=signal,daisy=daisy,drift=drift
...
if dname eq 'june14' and field eq '1hr' then begin ; changed all this -km
   ffdir=['49-56','57-64','65-72','73-80','81-88','89-96','97-104','105-112','113-120','121-128','129-136','137-144','145-152','153-160']
endif
if dname eq 'june14' and field eq '22hr' then begin ; changed all this -km
   ffdir=['9-16','17-24','25-32','33-40','41-48']
endif


; These lines will be in constant flux until we figure out a better system.
;raidgmrt = '/mnt/raid-project/gmrt' ; added this -km
; raidgmrt = /cita/d/raid-project/gmrt
raidgmrt = '/cita/d/raid-cita/tchang/'
datadir='/cita/d/raid-project/gmrt/'

set_plot,'ps'
device,/color, bits_per_pixel=8

; GBT parameters
beam=15./60.d0 ; beam size FWHM
dra=beam/2.d ; I think this is pixel size in the ra direction ? -km
ddec=beam/2.d ; I think this is the pixel size in the dec direction ? -km
;dra=3./60.d0 ;arcmin
;ddec=3./60.d0
d0=1450.  ;2Mpc per redshift bin starting at D=1450 h^-1 Mpc
dd=2. ; pizel size im Mpc in the z dierection.
nz=1120/dd ; number of z pixels.
; Given our wide bandwidth, it is not technically right to linearly
; assign destances to frequencies... but this is probably good enough. -km
; Better fix this if you want to do Redshift s[ace distortions, this could
; since you really do want to be comparing power at the same scales.

freq_rest=1420.405751 ;MHz, z=0 frequency of the 21cm line
nfreq=2048
Npol=4
...
fbin=[40,20]
if field eq '1hr' then f0=[15,0]
if field eq '22hr' then f0=[325.0,0]
;fbin=[24,24]
;
;
...
nscan=8 ; Changed from 9 -km
;nscan=1
ntime=60 ; changed from 255 -km
npol=4 ; xx, yy, Re(xy), Im(xy)?
nwin=8 ; NUmber of windows (IF channels?)
nfreq=2048 ; number of frequencies per window.
;nn=16320L  ;255.*8*8  8 freq windows and 4 pol x 2 on-off
nn = ntime*nwin*npol*2 ; Probably pertains to the number of arrays to be read?
;
for ifile=0,nfile-1 do begin
   ; Figure out fits file names for reading.
   ;
...
;   fn='/cita/d/raid-cita/tchang/GBT/GBT09C_075/00_zCOSMOS_drift_'+ffdir(ifile)+'.raw.acs.fits'
   ; I changed this line -km
   fn=datadir+'/pen/GBT10B_036/00_wigglez'+field+'_'+obsmode+'_'+ffdir(ifile)+'.raw.acs.fits'
   ;
;   nscan=1
   ;if ffdir(ifile) eq 'scan27-28' then nscan=2
   ;if ffdir(ifile) eq '30' then nscan=1
   ;
   ; Allowcate memory for maps for storing:
   radio=dblarr(Npol,nx,ny,nz) ; raw radio map?
   weight=radio ; varience map?
   radio20mode=dblarr(Npol,nx,ny,nz) ; radio map with 20 svd modes removed?
   weight20mode=radio ; corresponding varience?
   ...
   weight0mode=radio
   noiseradio=dblarr(Npol,nx,ny,nz) ; noise maps?  
   ; how is this different that weight?
   ...
   ;
   ;
   time_freq_tot=dblarr(ntime*nscan,npol,nfreq*nwin) ; holds time series data?
   freqkeep=dblarr(nfreq*nwin) ; frequencies to not be thrown away?
   time_freq_matrix=dblarr(ntime*nscan,npol,nfreq*3/5*nwin) ; no idea
   ; what this is for. -km
   ; is 3/5 the fraction of the windows that don't overlap? -km
   ...
   ratot=dblarr(ntime*nscan) ; the pointing as function of time (Ra)
   dectot=dblarr(ntime*nscan) ; Dec
   ;
   flagnumon=0L
   flagnumoff=0L
   flagnumoff20mode=0L
   ...
   flagnumoff0mode=0L
   ;
   ; Read fitsfile data.  Otherwise, much of the work already done
   ; and saved elsewhere.
   if keyword_set(fitsfile) then begin
      ;
      for scan=0L,nscan-1L do begin ;loop over scans
         ...
         ; each scan contains nn records.  Figure out which records to read.
         rec0=nn*scan ; First record to be read in this iteration
         rec1=nn*(scan+1)-1L ; last one
         ;
         fdir=raidgmrt+'/wiggleZ/processed/'+dname+'/'+ffdir(ifile)+'/'
         print,'fdir: ',fdir
         print,'fn: ',fn
         ...
         zmin_rec=nz
         ;
         for i=0,nwin-1 do begin ; loop over windows.
            ;
            ; Read a single IF from a single scan from the fits file
            ; and do a bunch of cuts and such.
            ;
            ; t_off, t_on, noise, flag_off, flage_on, dec, pol all
            ; new arrays for storeing data.  Not previously declaired
            ...
            ;
            ; Now we have the time series data, devided into bins where the
            ; calibrator is on and off.
            ;
            ; Reorgainizing data.
            ; Throw away a bunch of polarization data 
            ; and instead devide things by the noise calibrator On-Off.
            ...
            tbinnoise(*,3,*)=noise(*,3,*)
            ;
            ; print total number of records flagged and fraction
            ind=where(tbinflag eq 0,cind)
            print,'flagged:',cind,double(cind)/n_elements(tbinflag)
            ;
            ; print total number recods flagged but don't multiply
            ...
            ; do bandpass correction0
            ; calculate the bandpass by taking time median per pol per freq
            ;
            ; At each polarization and frequency, find the time median of
            ; unflagged intensities and store it in band pass.
      ...
      ;
      device,filename=fdir+'Psky_freq_matched.ps'
      ...
      ;
      ;
      device,filename=fdir+'Pnoise_freq_matched.ps'
      ...
      ;
      fdir2=raidgmrt+'/wiggleZ/processed_nomeansub/'+dname+'/'+ffdir(ifile)+'/'
      ; read in the data files
      openr,1,fdir2+'radio_tf_b4svd'
      readf,1,time_freq_flag
      close,1
      ; 
      openr,1,fdir2+'flagtot_tf_b4svd'
      readf,1,flagtot
      close,1
      ;
      openr,1,fdir2+'noisecal_tf_b4svd'
      readf,1,time_freq_noise
      close,1
      ;
      openr,1,fdir2+'freqtot'
      readf,1,freqtot
      close,1
      ;
      openr,1,fdir2+'ra'
...
;      endfor
                                ;
;      fdir='/cita/d/raid-cita/tchang/GBT/Processed_sim_'+signal+'/'+dname+'/'+field+'_'+ffdir(ifile)+'/'
      fdir='/cita/d/raid-project/gmrt/tchang/GBT/zCOSMOS/Processed_sim_'+signal+'/'+dname+'/'+ffdir(ifile)+'/' 
      spawn,'mkdir -p '+fdir
      ;
   endif
   ;
   ; for constant elevation scans, subtract off the mean of the same-direction scans
   ; odd scans
   flagtmp=flagtot
   ind=where(flagtot ne 1,cind)
   if cind gt 0 then flagtmp(ind)=0
   time_freq_tmp=time_freq_flag*flagtmp
   time_freq_tmp=reform(time_freq_tmp,nscan,ntime,npol,nfreq*3/5*nwin)
   time_freq_flag=reform(time_freq_flag,nscan,ntime,npol,nfreq*3/5*nwin)
   ;
   scanindx=indgen(nscan)
   indo=where(scanindx mod 2 eq 0,cindo)
   inde=where(scanindx mod 2 eq 1,cinde)
   time_freq_odd=reform(time_freq_tmp(indo,*,*,*))
   time_freq_even=reform(time_freq_tmp(inde,*,*,*))
   odd_mean=total(time_freq_odd,1,/cumulative)/double(cindo)
   even_mean=total(time_freq_even,1,/cumulative)/double(cinde)
   help,odd_mean
   help,even_mean
   help,time_freq_flag
   for nindo=0,cindo-1 do $
      time_freq_flag(indo(nindo),*,*,*)=time_freq_flag(indo(nindo),*,*,*)-odd_mean(cindo-1,*,*,*)
   for ninde=0,cinde-1 do $
      time_freq_flag(inde(ninde),*,*,*)=time_freq_flag(inde(ninde),*,*,*)-even_mean(cinde-1,*,*,*)
   time_freq_flag=reform(time_freq_flag,nscan*ntime,npol,nfreq*3/5*nwin)
   ;
 ...
 ;     freqnum=freqnum>1
      ...
      ;
      device,filename=fdir+'Psky_svd1.ps'
      ...
      ;
      ;
      device,filename=fdir+'Psky_svd1.scut.ps'
      ;
      tfradio=dblarr(ntime*nscan,nz)
      ...
      ;
      ;
      openw,2,fdir+'timevec_5mode'
      openw,3,fdir+'timevec_10mode'
      openw,4,fdir+'timevec_15mode'
      openw,5,fdir+'timevec_20mode'
      openw,6,fdir+'timevec_25mode'
      openw,12,fdir+'freqvec_5mode'
      openw,13,fdir+'freqvec_10mode'
      openw,14,fdir+'freqvec_15mode'
      openw,15,fdir+'freqvec_20mode'
      openw,16,fdir+'freqvec_25mode'
      openw,22,fdir+'eigenvalue_5mode'
      openw,23,fdir+'eigenvalue_10mode'
      openw,24,fdir+'eigenvalue_15mode'
      openw,25,fdir+'eigenvalue_20mode'
      openw,26,fdir+'eigenvalue_25mode'
      for ii=0,npol-1 do begin
         ;
         radio2d=reform(time_freq_flag(*,ii,*))
         la_svd,radio2d, s,u,v,/double,status=status
      ...
      ;
      ;
      device,filename=fdir+'Psky_svd2.ps'
      ;
      tfradio=dblarr(ntime*nscan,nz)
      thermal=(dnu*dt)*(flagtot0mode)
      for ifreq=0,nz-1 do begin
      ...
      ;
      ;
      device,filename=fdir+'Pnoise_flag.ps'
      plt_image,transpose(reform(time_freq_noise(*,0,*))),/scalable,/colbar
      plt_image,transpose(reform(time_freq_noise(*,1,*))),/scalable,/colbar
      plt_image,transpose(reform(time_freq_noise(*,2,*))),/scalable,/colbar
      plt_image,transpose(reform(time_freq_noise(*,3,*))),/scalable,/colbar
   ...
   if not keyword_set(svdfg) then begin
      ...
      mk_hicube_half,radiores15mode,flagtot,time_freq_noise,ratot,dectot,freqtot,pol,$
                      parallatic,radio=radio,weight=weight,$
                      noiseradio=noiseradio,field=field,flagnum=flagnumoff,$
                     dra=dra,ddec=ddec,f0=f0,fbin=fbin
   ...
   endelse
;
   device,filename=fdir+'Psky_xz_aftsvd.ps'
   ;
   ;
   radio2=radio0mode
   ...
   ;
   ;
   openw,1,fdir+field+'.radio_aftsvd'
   printf,1,radio
   close,1
   ;
   openw,1,fdir+field+'.weight_aftsvd'
   printf,1,weight
   close,1
   ;
   openw,1,fdir+field+'.radio20mode_aftsvd'
   printf,1,radio20mode
   close,1
   ;
   openw,1,fdir+field+'.weight20mode_aftsvd'
   printf,1,weight20mode
   close,1
   ;
   openw,1,fdir+field+'.radio25mode_aftsvd'
   printf,1,radio25mode
   close,1
   ;
   openw,1,fdir+field+'.weight25mode_aftsvd'
   printf,1,weight25mode
   close,1
   ;
   openw,1,fdir+field+'.radio10mode_aftsvd'
   printf,1,radio10mode
   close,1
   ;
   openw,1,fdir+field+'.weight10mode_aftsvd'
   printf,1,weight10mode
   close,1
   ;
   openw,1,fdir+field+'.radio5mode_aftsvd'
   printf,1,radio5mode
   close,1
   ;
   openw,1,fdir+field+'.weight5mode_aftsvd'
   ...
   radio2(ind)=radio2(ind)/weight(ind)
   ;
   openw,1,fdir+field+'.radio_weighted_aftsvd'
   printf,1,radio2
   close,1
   ;
   radio2=radio20mode
   ...
   radio2(ind)=radio2(ind)/weight20mode(ind)
   ;
   openw,1,fdir+field+'.radio20mode_weighted_aftsvd'
   printf,1,radio2
   close,1
   ;
   radio2=radio25mode
   ...
   radio2(ind)=radio2(ind)/weight25mode(ind)
   ;
   openw,1,fdir+field+'.radio25mode_weighted_aftsvd'
   printf,1,radio2
   close,1
   ;
   radio2=radio10mode
   ...
   radio2(ind)=radio2(ind)/weight10mode(ind)
   ;
   openw,1,fdir+field+'.radio10mode_weighted_aftsvd'
   printf,1,radio2
   close,1
   ;
   radio2=radio5mode
   ...
   radio2(ind)=radio2(ind)/weight5mode(ind)
   ;
   openw,1,fdir+field+'.radio5mode_weighted_aftsvd'
   printf,1,radio2
   close,1
   ;
   radio2=radio0mode
...
openw,1,fdir+field+'.radiotot_aftsvd'
printf,1,radiotot
close,1
;
openw,1,fdir+field+'.weighttot_aftsvd'
printf,1,weighttot
close,1
;
...
;
openw,1,fdir+field+'.radiotot_weighted_aftsvd'
printf,1,radiotot
close,1
;
;

openw,1,fdir+field+'.radiotot20mode_aftsvd'
printf,1,radiotot20mode
close,1
;
openw,1,fdir+field+'.weighttot20mode_aftsvd'
...
;
openw,1,fdir+field+'.radiotot20mode_weighted_aftsvd'
printf,1,radiotot20mode
close,1
;
openw,1,fdir+field+'.radiotot25mode_aftsvd'
printf,1,radiotot25mode
close,1
;
openw,1,fdir+field+'.weighttot25mode_aftsvd'
...
;
openw,1,fdir+field+'.radiotot25mode_weighted_aftsvd'
printf,1,radiotot25mode
close,1
;
;
openw,1,fdir+field+'.radiotot10mode_aftsvd'
printf,1,radiotot10mode
close,1
;
openw,1,fdir+field+'.weighttot10mode_aftsvd'
...
;
openw,1,fdir+field+'.radiotot10mode_weighted_aftsvd'
printf,1,radiotot10mode
close,1
;
;
openw,1,fdir+field+'.radiotot5mode_aftsvd'
printf,1,radiotot5mode
close,1
;
openw,1,fdir+field+'.weighttot5mode_aftsvd'
...
;
openw,1,fdir+field+'.radiotot5mode_weighted_aftsvd'
printf,1,radiotot5mode
close,1
;
;
...
;
;
device,filename=fdir+'Pskytot_aftsvd.ps'
;
