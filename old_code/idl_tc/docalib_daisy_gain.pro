pro docalib_daisy_gain,radio,weight,t_off,radio_on,weight_on,field=field,fn=fn,sigmacut=sigmacut,dname=dname,svdfg=svdfg,fitsfile=fitsfile
...
;
if dname eq 'friday' then begin
   ffdir=['284_288','289-293','294_298','299_303','304-308'] ;sept06
endif
if dname eq 'aug29' then begin
...
;   fsdir='/cita/scratch/cottontail/tchang/GBT/Processed/'+dname+'/svd_nominus1/'+field+'_'+ffdir(ifile)+'/'
   spawn,'mkdir '+fdir
   if field eq 'f3' then fn='/cita/scratch/bear/pen/GBT/DATA/GBT08A_081/'+dname+'/field3_daisy_'+ffdir(ifile)+'.raw.acs.fits'
   if field eq 'f4' then fn='/cita/scratch/bear/pen/GBT/DATA/GBT08A_081/'+dname+'/field4_daisy_'+ffdir(ifile)+'.raw.acs.fits'
   ;
   radio=dblarr(Npol,nx,ny,nz)
   weight=radio
   noiseradio=dblarr(Npol,nx,ny,nz)
...
;      device,filename=fdir+'bandpass'+strtrim(string(scan),2)+'.ps'
      ...
      for i=0,nwin-1 do begin
         ...
         calibration_svd,t_off,t_on,noise,flag_off,flag_on,ra,dec,freq,pol,parallatic,recn=[rec0,rec1],thisif=i,threshould=0.02,fn=fn,/daisy 
