PRO correlate_combined,radio,weight,optical,field=field,fin=fin,radiocut=radiocut,fdir=fdir,noise=noise,sim=sim,daisy=daisy,outfile=outfile,dname=dname
...
;
radioall=dblarr(nday,npol,nx,ny,nz)
weightall=radio
Da_z,oz,dist
...
for i=0,nday-1 do begin
   ...
   openr,1,ffdir(i)+field+'.Tsky_svd_5mode'
   readf,1,tmp
   close,1
   radioall(i,*,*,*)=tmp
   ;
   openr,1,ffdir(i)+field+'.weight_svd_5mode'
   readf,1,tmp
   close,1
   weightall(i,*,*,*)=tmp
   ;
