PRO correlate_gain_svdtest,corrv1,corrv2,corre1,corre2,corre3,corrm1,corrm2,radio,weight,optical,field=field,fin=fin,radiocut=radiocut,fdir=fdir,noise=noise,sim=sim,daisy=daisy,outfile=outfile,dname=dname,svdfg=svdfg,mode=mode,combine=combine,svdmode=svdmode,test=test,mask=mask,dosigcut=dosigcut,residual=residual,onlysim=onlysim,sigcut5=sigcut5
...
if keyword_set(sigcut5) then sigcut5='_5sigcut' else sigcut5=''
...
neweight=reform(neweight,nx,ny,nz)
   ...
   if keyword_set(residual) then begin
      ;
      radiopol=dblarr(npol,nx,ny,560)
      radiopol2=radiopol
      radiopol1=radiopol
      ;radio=dblarr(nx,ny,nz)
     ; openr,1,'/cita/d/raid-cita/tchang/GBT/Processed_mode/aug29/f3_05-10/f3.svdresidual_weighted_'+mode
;      openr,1,field+'.residual1mode.dat'
;      readf,1,radio
      if field eq 'f4' then begin
      openr,1,'/cita/d/raid-cita/tchang/GBT/Processed'+sigcut5+'/aug29/f4_63-68/f4.svddiff_weighted_'+mode
      readf,1,radiopol1
      close,1
      ;
      openr,1,'/cita/d/raid-cita/tchang/GBT/Processed_mode_simonly_100muK/aug29/f4_63-68/f4.radio0mode_weighted_aftsvd'
      readf,1,radiopol2
      close,1
      ;
   endif 
      ;
      if field eq 'f3' then begin
      openr,1,'/cita/d/raid-cita/tchang/GBT/Processed'+sigcut5+'/aug29/f3_05-10/f3.svddiff_weighted_'+mode
      readf,1,radiopol1
      close,1
      ;
      ...
      radiopol=radiopol2-radiopol1
      ;
   endif else begin
      radiopol=dblarr(npol,nx,ny,560)
      openr,1,'/cita/d/raid-cita/tchang/GBT/Processed'+sigcut+'/aug29/f3_05-10/f3.radio'+mode+'_weighted_aftsvd'
      readf,1,radiopol
      close,1
      ;
      radiopol2=dblarr(npol,nx,ny,560)
      openr,1,'/cita/d/raid-cita/tchang/GBT/Processed_sim_100muK'+sigcut+'/aug29/f3_05-10/f3.radio'+mode+'_weighted_aftsvd'
      readf,1,radiopol2
      close,1
      ;
      radiopol=radiopol2-radiopol
      ;
   endelse
   ;
   if keyword_set(onlysim) then begin
      ;
      radiopol=dblarr(npol,nx,ny,560)
      openr,1,'/cita/d/raid-cita/tchang/GBT/Processed_mode_simonly_100muK/aug29/f3_05-10/f3.radio'+mode+'_weighted_aftsvd'
      readf,1,radiopol
      close,1
      ;
   endif
   ;
   radiopol=radiopol(*,*,*,2+185:555+2-1)
   radio=reform(radiopol(0,*,*,*))
   help,radio
   ;   
   gainx=dblarr(npol,nx*ny)
   gainz=dblarr(npol,nz)
   ...
   for ix=0,nx*ny-1 do begin
      for iz=0,nz-1 do begin
         if (gainx(ix)*gainz(iz) ne 0.) then begin
            radio(ix,iz)=radio(ix,iz)/gainx(ix)/gainz(iz) 
         endif
      endfor
   endfor
   radio=reform(radio,nx,ny,nz)
   ...
   ;
;   openw,1,field+'.svdtest.onlysim.dat'
;   printf,1,radio
;   close,1
   ;
...
;   close,1
   ;
   openr,1,'~/projects/GBT/pros/new_weight/'+field+'.diffnewweight.'+mode+sigcut5+'.dat'
   readf,1,neweight
   close,1
   ;
   openr,1,'~/projects/GBT/pros/new_weight/'+field+'.diffweight.'+mode+sigcut5+'.dat'
   readf,1,weight
   close,1
   ;
