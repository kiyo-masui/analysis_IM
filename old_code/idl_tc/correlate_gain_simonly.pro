PRO correlate_gain_simonly,corrv1,corrv2,corre1,corre2,corrm1,corrm2,radio,weight,optical,field=field,fin=fin,radiocut=radiocut,fdir=fdir,noise=noise,sim=sim,daisy=daisy,outfile=outfile,dname=dname,svdfg=svdfg,mode=mode,combine=combine,dosigcut=dosigcut,mask=mask
...
;   thisdir='/cita/d/raid-cita/tchang/GBT/Processed/'
   sdir='/cita/d/raid-cita/tchang/GBT/Processed_mode_simonly_'+signal+'/'
   thisdir='/cita/d/raid-cita/tchang/GBT/Processed_mode/'
...
for iday=0,nday-1 do begin
   ...
   ;
; original
   openr,1,fsdir(iday)+field+'.radiotot'+mode+'_weighted_aftsvd'
   readf,1,radio
   close,1
   ;
   openr,1,fsdir(iday)+field+'.weighttot'+mode+'_aftsvd'
   readf,1,weight
   close,1
   ;
...
;
; write outputs
openw,1,fsdir(nday-1)+field+'.radio.sim.combined_'+signal
printf,1,radio
close,1
;
openw,1,fsdir(nday-1)+field+'.weight.sim.combined_'+signal
printf,1,weight
close,1
;
radioweight=dblarr(nx,ny,nz)
openr,1,ffdir(nday-1)+field+'.radio.processed.combined'+mode+'svd'
readf,1,radioweight
close,1
;
openr,1,ffdir(nday-1)+field+'.weight.processed.combined'+mode+'svd'
readf,1,weight
close,1
;
...
device,filename=field+'.skymapsim'+mode+'_'+signal+'.ps'
plt_image,transpose(radio2d),/scalable,/colbar
plt_image,transpose(weight2d),/scalable,/colbar
plt_image,transpose(neweight),/scalable,/colbar
plt_image,transpose(radio2d*weight2d),/scalable,/colbar
ind=where(weight2d eq 0,cind)
if cind gt 0 then radio2d(ind)=0
plt_image,transpose(radio2d*neweight),/scalable,/colbar
