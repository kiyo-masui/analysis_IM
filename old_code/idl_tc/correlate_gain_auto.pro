PRO correlate_gain_auto,corrv1,corrv2,corre1,corre2,corre3,corrm1,corrm2,radio,weight,optical,field=field,fin=fin,radiocut=radiocut,fdir=fdir,noise=noise,sim=sim,daisy=daisy,outfile=outfile,dname=dname,svdfg=svdfg,mode=mode,combine=combine
...
;
; write outputs
;openw,1,ffdir(nday-1)+field+'.radio.processed.combined'
;printf,1,radio
;close,1
;
;openw,1,ffdir(nday-1)+field+'.weight.processed.combined'
;printf,1,weight
;close,1
;
...
openw,1,'corrlag.auto.'+field+mode+'.dat' ;,/append
printf,1,corrlagvalue
close,1

openw,1,'correrr.auto.'+field+mode+'.dat' ;,/append
printf,1,corrlagerr
close,1

openw,1,'corrcovar.auto.'+field+mode+'.dat'; ,/append
printf,1,corrcovar
close,1
