pro calibration_svd,Tsky_off,Tsky_on,noisecal,noise_time,noise_bpass,flagtot_off,flagtot_on,$
...
; now deal with one If at a time
data=data(ind)
...
freq=((dindgen(nfreq)-nfreq/2)*data(0).cdelt1/1e6)+double(dif(thisif))
Tsky=dblarr(nave*tave,nrec,nfreq)
Televation=dblarr(nave*tave)
...
noise_on=noise_time
noise=dblarr(nave*tave,nrec/2,nfreq)
noiseflag=noise
noisetot=noise
;
; do median in time
for j=0,nrec-1,2 do begin
   ...
   for k=0,nfreq-1 do begin     
      ...
      ind1=where(flagtot(*,j+1,k) eq 1,cind)
      ind2=where(flagtot(*,j,k) eq 1,cind)
      if cind gt 1 then begin
         noise_time(0,j/2,k)=median(medd(ind),/double)
         noise_bpass(0,j/2,k)=median(medd2(ind1),/double)
         noise_on(0,j/2,k)=median(medd3(ind2),/double)
...
help,noise_bpass
;set_plot,'ps'
;device,filename='bpass'
plot,reform(noise_time(0,0,*))
plot,reform(noise_time(0,1,*))
plot,reform(noise_time(0,2,*))
plot,reform(noise_time(0,3,*))
plot,reform(noise_bpass(0,0,*))
oplot,reform(noise_on(0,0,*)),linestyle=2
plot,reform(noise_bpass(0,1,*))
oplot,reform(noise_on(0,1,*)),linestyle=2
plot,reform(noise_bpass(0,2,*))
oplot,reform(noise_on(0,2,*)),linestyle=2
plot,reform(noise_bpass(0,3,*))
oplot,reform(noise_on(0,3,*)),linestyle=2
;device,/close
