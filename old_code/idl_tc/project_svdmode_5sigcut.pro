pro project_svdmode_5sigcut,iradio,field=field,sim=sim,rebin=rebin
...
;   thisdir='/cita/d/raid-cita/tchang/GBT/Processed_mode_simonly_'+signal+'/' else thisdir='/cita/d/raid-cita/tchang/GBT/Processed/'
   if keyword_set(sim) then thisdir='/cita/d/raid-cita/tchang/GBT/Processed_mode_simonly_'+signal+'/' else thisdir='/cita/d/raid-cita/tchang/GBT/Processed_5sigcut/'
...
;   subfmdir='/cita/d/raid-cita/tchang/GBT/Processed_mode/'+subdir
   subfmdir='/mnt/raid-cita/tchang/GBT/Processed_5sigcut/'+subdir
   nx=40
   ny=6
   ;
endif
...
;   thisdir='/cita/d/raid-cita/tchang/GBT/Processed_mode_simonly_'+signal+'/' else thisdir='/cita/d/raid-cita/tchang/GBT/Processed/'
   ...
   submapdir=thisdir+subdir
;   subfmdir='/cita/d/raid-cita/tchang/GBT/Processed_mode/'+subdir
   subfmdir='/cita/d/raid-cita/tchang/GBT/Processed_5sigcut/'+subdir
   nx=30
   ny=6
   ;
endif
...
for ifile=0,0 do begin
   ;
   device,filename=field+'.residual.5sigcut.'+strtrim(string(ifile),2)+'.ps'
...
endelse
   ...
   for imode=0,nmode-1 do begin
      ...
      fmmdir=fmdir
      if (mode(imode) eq 7 or mode(imode) eq 8 or mode(imode) eq 9 or mode(imode) eq 11 $
          or mode(imode) eq 12) then begin
