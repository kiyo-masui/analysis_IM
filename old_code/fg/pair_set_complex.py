...
class PairSet_Complex(pair_set_legendre.PairSet_LegendreSVD):
    ...
    @batch_handler.log_timing
    def complex_correlation(self):
        if not 'I_with_Q' in self.pairlist: 
            print 'Error: Map I_with_Q missing'
            exit()
        if not 'I_with_U' in self.pairlist:
            print 'Error: Map I_with_U missing'
            exit()

        par = self.params

        filename = self.output_root
        filename_corr_real = filename + "foreground_corr_pair_I_with_Q.pkl"
        filename_corr_imag = filename + "foreground_corr_pair_I_with_U.pkl"

        filename_corr_pol = filename + "foreground_corr_pair_I_with_P.pkl"

        if os.access(filename_corr_real, os.F_OK):
            (freq_cov_real, counts_real) = ft.load_pickle(filename_corr_real)
        else:
            print "ERROR: correlation functions not loaded"
            sys.exit()
        if os.access(filename_corr_imag, os.F_OK):
            (freq_cov_imag, counts_imag) = ft.load_pickle(filename_corr_imag)
        else:
            print "ERROR: correlation functions not loaded"
            sys.exit()
        freq_cov = freq_cov_real - 1j*freq_cov_imag
        counts = counts_real
        ft.save_pickle((freq_cov, counts), filename_corr_pol)

        pairdict = par['pairdict']
        mapq = algebra.make_vect(algebra.load(pairdict['I_with_Q']['map2']))
        mapu = algebra.make_vect(algebra.load(pairdict['I_with_U']['map2']))
        mapp = mapq + 1j*mapu
        mapp.info = mapq.info
        mapp_name = pairdict['I_with_Q']['map2'].replace('Q', 'P')
        algebra.save(mapp_name, mapp)

        #mapi = algebra.make_vect(algebra.load(pairdict['I_with_Q']['map1']))
        #weti = algebra.make_vect(algebra.load(pairdict['I_with_Q']['noise_inv1']))
        #wetp = algebra.make_vect(algebra.load(pairdict['I_with_Q']['noise_inv2']))

        #self.pairlist += 'I_with_P'
        #pair = map_pair.MapPair(mapi, mapp, weti, wetp, self.freq_list)
        #pair.set_names('I', 'P')
        #pair.params = self.params
        #self.pair['I_with_P'] = pair

        par['pairlist'] += ['I_with_P',]
        print par['pairlist']
        pairdict['I_with_P'] = {}
        pairdict['I_with_P']['map1'] = pairdict['I_with_Q']['map1']
        pairdict['I_with_P']['map2'] = mapp_name
        pairdict['I_with_P']['noise_inv1'] = pairdict['I_with_Q']['noise_inv1']
        pairdict['I_with_P']['noise_inv2'] = pairdict['I_with_Q']['noise_inv2']
        pairdict['I_with_P']['tag1'] = 'I'
        pairdict['I_with_P']['tag2'] = 'Q'
        par['pairdict'] = pairdict
