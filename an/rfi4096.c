...
/*   A similar computation of the rms.    */				
	...
	for(l=0; l<big_number; l++)  {     /* never ending loop is too risky. */
		...
		/*   Recompute rms and mean ignoring suspected noise. Set your threshold. */					
			if(sq(array[i]-mu) < sq(threshold*lim))
			{  count += 1.; rms1 += sq(array[i]-mu);  }
		rms1 = sqrt(rms1/count);			
		
		count = 0; mu = 0.;
		for(i=0; i<len; ++i)
			if(sq(array[i]) < sq(threshold*lim))
				{  count += 1.; mu += array[i];  }
		mu /= count;

		if( absl((rms1-lim)/lim) < when_to_quit ) break;  /* When to exit. */
		lim = rms1;	
	}	
...
void get_fit4096(double array[POINTS], double f[POINTS], double fit_array[POINTS])    {
...
void clean4096(double sig, int tol, int flat, int spike, int dTdf_limit, int dTdf_tol, double fit[POINTS], double cross1[POINTS], double array1[POINTS], double f1[POINTS], int m[POINTS])  { 
