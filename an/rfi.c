...
void testing(double A) { printf("Hi\n"); }
void testing2(double A, double *B) { printf("Hi\n"); }
...
#define PARTS 4    
	...
	for(i=0; i<parts; i++)    {     /* Fit a straight line */		

		x_bar += (nu[i]/d_parts); y_bar += (T[i]/d_parts);		
		sum_x_x += (nu[i]*nu[i]); sum_x_y += (nu[i]*T[i]);				
	}
...
void get_fit(double array[POINTS], double f[POINTS], double fit_array[POINTS])    {
	...
	/* Change  NUM_PIECES and BLOCK as required. NUM_PIECES * BLOCK = 2048 */
	 
	#define  NUM_PIECES 32         /* Divide into 32 parts.  */
	#define  BLOCK 64              /* 2048/32 = 64 */
	
	int num_pieces = (int)(NUM_PIECES), block = (int)(BLOCK),i,j;
	double pieces[NUM_PIECES][BLOCK], f_pieces[NUM_PIECES][BLOCK], fit[BLOCK],rms,mean, temp_fit[POINTS];
...
void flatten(double array[POINTS], double fit_array[POINTS])    {
...
void mask_array(int size, double array[POINTS], double sig, int mask[POINTS], int tol)    {
...
void rfi_find_dTdf(double nu[POINTS], double arr[POINTS], int mask[POINTS], int count, int lim)    {
...
void hi_f_spikes(double array[POINTS], double f[POINTS], int lim, int count, int tol)      {
...
void rfi_flag(int flat, int spike, int tol, double sig, double fit[POINTS], double array[POINTS], double f[POINTS], int mask[POINTS])  {
...
void clean(double sig, int tol, int flat, int spike, int dTdf_limit, int dTdf_tol, double fit[POINTS], double cross1[POINTS], double array1[POINTS], double f1[POINTS], int m[POINTS])  { 
