...
## If python complains of indentation errors, use tabs after def, if, for,
## etc... not spaces.
...
def	interpolate_weights(axes, pnt, x0, step_sizes):
	...
	return np.array(needed_nodes),weights
...
def interpolate(axes, arr, pnt, x0, step_sizes):
	...
	if len(axes) == arr.ndim:
		...
		points, weights = interpolate_weights(axes, pnt, x0, step_sizes)
